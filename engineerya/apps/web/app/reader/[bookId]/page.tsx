...
export default function DocumentReaderPage() {
  ...
  const totalPages = 15; // Simulated preview count
  ...
  return (
    <div className="flex flex-col h-screen bg-[#070A13] text-[#E2E8F0] select-none">
      {/* Top Bar Header */}
      <header className="h-14 bg-[#0F1424] border-b border-slate-900 px-4 flex items-center justify-between z-10">
        <div className="flex items-center space-x-4">
          <Link href="/books" className="text-sm font-semibold text-slate-400 hover:text-white transition-colors flex items-center space-x-1">
            <span>&larr;</span>
            <span className="hidden sm:inline">Close Reader</span>
          </Link>
          <span className="text-slate-600">|</span>
          <h2 className="text-sm font-bold text-white max-w-xs truncate">
      ...
      <div className="flex-grow overflow-auto p-4 md:p-8 flex items-center justify-center relative bg-[#070A13]">
        
        {/* Document Rendering Canvas Container */}
        <div className="relative aspect-[3/4] w-full max-w-xl bg-white text-slate-900 rounded-lg shadow-2xl p-8 md:p-12 overflow-hidden flex flex-col justify-between select-none pointer-events-none">
          
          {/* SECURE DYNAMIC WATERMARK OVERLAYS (Simulating Phase 6 backend JPG composition) */}
          <div className="absolute inset-0 grid grid-cols-2 grid-rows-3 opacity-[0.04] select-none pointer-events-none rotate-[-25deg] scale-125">
            {[...Array(6)].map((_, i) => (
          ...
          <div className="border-b border-slate-200 pb-3 flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-wider">
          ...
          <div className="my-6 flex-grow">
            ...
            <p className="text-xs leading-relaxed text-slate-750 mb-3">
          ...
          <div className="border-t border-slate-200 pt-3 flex justify-between items-center text-[9px] text-slate-400 font-medium">
            ...
            <span className="italic text-slate-500">Watermarked Delivery Session ID: WS_{bookId?.toString().substring(0, 8)}</span>
          </div>
        </div>
      </div>
    </div>
